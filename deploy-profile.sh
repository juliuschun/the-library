...
if cmd == "profile-tags":
...
elif cmd == "scoped-library-yaml":
    # Generate a scoped library.yaml for a customer VM — keeps only the
    # deployed profile + its skills. Omits the `customers:` block (SSH IPs
    # and fleet metadata are operator-private).
    #
    # Tower's packages/backend/services/library-skills.ts reads this file
    # at boot and cross-checks every entry against ~/.claude/skills/<name>/
    # SKILL.md. Entries without a matching folder are silently dropped, so
    # shipping a slightly over-broad list is still safe — but we filter by
    # profile tags anyway to keep the file tidy and self-describing.
    profile_name = sys.argv[3]
    tags = json.loads(sys.argv[4])
    tag_set = set(tags)
    profiles = data.get("profiles", {})
    skills = data.get("library", {}).get("skills", [])
    matched = [s for s in skills if tag_set.intersection(set(s.get("tags", [])))]
    scoped = {
        "default_dirs": data.get("default_dirs", {}),
        "profiles": {
    ...
    header = (
        "# Auto-generated by deploy-profile.sh — DO NOT EDIT.\n"
        f"# Profile: {profile_name}\n"
        "# Customers block intentionally omitted on customer VMs.\n"
        "# Regenerate by re-running deploy-profile.sh from the Moat AI operator host.\n\n"
    )
    print(header + yaml.safe_dump(scoped, sort_keys=False, allow_unicode=True, width=120))
...
# per-skill SKILL.md as the single source of truth for company skills.
# Without library.yaml on the customer VM, bootstrapLibraryProviders
# reports library=0 even when all SKILL.md folders are correctly present.
# See: workspace/decisions/2026-04-21-fresh-install-scaffolding-audit.md
SCOPED_YAML=$(parse_yaml "$YAML_FILE" scoped-library-yaml "$PROFILE" "$TAGS")
echo -ne "  Deploying scoped library.yaml..."
if echo "$SCOPED_YAML" | ssh "$SSH_TARGET" 'mkdir -p ~/.claude/skills/library && cat > ~/.claude/skills/library/library.yaml'; then
  echo -e " ${GREEN}✓${NC}"
else
  echo -e " ${RED}✗${NC}"
  warn "Scoped library.yaml deployment failed — bootstrapLibraryProviders will report library=0"
fi
